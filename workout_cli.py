...
class WorkoutApp:
    ...
    def start_workout(self):
        ...
        for i, exercise in enumerate(exercises, 1):
            ...
            while True:
                try:
                    reps = int(input(f"   How many {exercise.name.lower()} did you complete? "))
                    if reps >= 0:
                        break
    ...
    def show_progress(self):
        ...
        for category, exercises in self.exercise_db.exercises.items():
            ...
            for exercise in exercises:
                ...
                print(f"  {status} {exercise.name}: {exercise.max_reps_achieved} reps (Level {exercise.difficulty_level})")
    ...
    def show_workout_stats(self):
        ...
        if total_workouts > 0:
            first_workout = min(session.timestamp.date() for session in self.workout_history)
            last_workout = max(session.timestamp.date() for session in self.workout_history)
...
def main():
    parser = argparse.ArgumentParser(description='Workout Snacks - Calisthenics Tracker')
    parser.add_argument('command', nargs='?', choices=['workout', 'progress', 'charts', 'daemon'], 
